        ...
        <div className="w-full gap-2 flex flex-col">
          ...
          <div className="w-full bg-[#D4DDE6] h-[40px] px-4 flex justify-between items-center ">
            ...
            <button
              ...
              className="px-3 py-1 csm:block hidden bg-[#1957EA] text-[14px] font-normal text-white-1/80 rounded-[5px]"
            >
              New Post
            </button>
            <button
              onClick={() => setNewPost(true)}
              className="min-w-[12px] csm:hidden flex min-h-[12px] rounded-[4px] border-[2px] border-black-1 justify-center items-center"
            >
              <FiPlus className="text-black-1 text-[16px]" />
            </button>
          </div>
          <div className="w-full h-[calc(100vh-200px)] gap-2 overflow-auto flex flex-col">
            {/* update bar ---> */}
            <div className="w-full mt-[2px] bg-black-1 px-2 csm:px-4 h-[40px] flex justify-between items-center ">
              <p className="text-[12px] csm:text-[14px] font-bold text-yellow-1">
                #7 Update: Bot development - dd/mm/yy hh:mm:ss
              </p>
              <button className="min-w-[20px] min-h-[20px] bg-yellow-1 flex justify-center items-center">
            ...
            <div className="w-full grid mt-2 gap-2 grid-cols-[3fr,1fr]">
              ...
              <div className="w-full flex gap-2 flex-col">
                ...
                <p className="text-[10px] csm:text-[12px] font-normal text-black-1 dark:text-white-1">{`<MAIN TEXT GOES HERE> is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem.`}</p>
              </div>
              {/* links  */}
                ...
                <div className="w-full justify-center items-center flex flex-col gap-2">
                  {links.map((link, index) => {
                    return (
                      <p
                        key={index}
                        className="text-black-1 dark:text-white-1 font-normal text-[12px] csm:text-[14px]"
                      >
                        {link}
                      </p>
                    );
              ...
              <div className="w-full flex flex-col gap-1">
                {[0, 1, 2, 3, 4].map((item, index) => {
                  return (
                    <div
                      key={index}
                      className="w-full mt-[2px] bg-black-1 px-2 csm:px-4 h-[40px] flex justify-between items-center "
                    >
                      <p className="text-[12px] csm:text-[14px] font-bold text-yellow-1">
